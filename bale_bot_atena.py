...
from utils import download_file, is_online
        ...
        try:
            ...
            if text == "/start":
                ...
                keyboard = {
                    "type": "keyboard",
                    "buttons": [
                        [{"type":"text","title":"ساخت مدل سه‌بعدی"}],
                ...
                self.send_message(chat_id, "سلام! چگونه می‌توانم کمک کنم؟", keyboard)
                return

            if state == "MAIN_MENU":
                if text == "ساخت مدل سه‌بعدی":
                    ...
                    self.send_message(chat_id, "با انتخاب 'ساخت مدل سه‌بعدی' عکس یا ویدیو ارسال کنید.\n"
            ...
            if state == "WAITING_FOR_KNOWLEDGE":
                ...
                self.send_message(chat_id, "دانش شما ذخیره شد. می‌توانید متن بیشتری ارسال کنید یا به منوی اصلی بازگردید.")
                return
            ...
            if state == "WAITING_FOR_MEDIA":
                    ...
                    if download_file(self.session, file_id, local_path):
                        ...
                        if success:
                            ...
                            if self.revenue.decrease_balance(chat_id, cost):
                                self.send_file(chat_id, result, caption="مدل سه‌بعدی شما")
                            else:
                ...
                if video:
                    ...
                    if download_file(self.session, file_id, local_path):
                        ...
                        if model_data:
                            ...
                            cost = 2
                            if self.revenue.decrease_balance(chat_id, cost):
                                self.send_file(chat_id, obj_path, caption="مدل سه‌بعدی ساخته شده از ویدیو")
                            else:
                ...
                if document and document.get("file_name", "").lower().endswith((".mp4", ".avi", ".mov")):
                    ...
                    local_path = os.path.join(OFFLINE_INPUT_DIR, f"video_{chat_id}_{int(time.time())}_{document['file_name']}")
                    if download_file(self.session, file_id, local_path):
                        model_data = self.engine.process_video_to_3d(local_path)
                        if model_data:
                            obj_path = os.path.join(OUTPUT_DIR, f"model_{chat_id}_{int(time.time())}.obj")
                            ...
                            cost = 2
                            if self.revenue.decrease_balance(chat_id, cost):
                                self.send_file(chat_id, obj_path, caption="مدل سه‌بعدی از فایل ویدیویی")
                            else:
                                self.send_message(chat_id, "موجودی کافی برای ساخت مدل ویدیویی ندارید.")
                        else:
                            self.send_message(chat_id, "خطا در ساخت مدل از ویدیو.")
            ...
            logger.error(f"Exception in handle_update extended: {e}", exc_info=True)
            try:
