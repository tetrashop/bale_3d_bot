...
MAX_RETRIES = 5
RETRY_BACKOFF = 2
MAX_WORKERS = 4  # تعداد Threadهای پردازش‌کننده پیام‌ها
...
class ChaosManager:
    ...
    برای جلوگیری از Race Condition و مدیریت صف پیام‌های ورودی، قفل‌ها، و مدیریت خطاها و بازیابی
    """
    def __init__(self):
...
class EconomicPlan:
    ...
    پیاده‌سازی محدودیت‌های اقتصادی مثل نرخ درخواست‌ها، محدودیت ذخیره‌سازی و مدیریت منابع
    """
    ...
    def can_process(self):
        with self.lock:
            ...
            # حذف درخواست‌های بیش از یک دقیقه گذشته
...
def get_image_data_from_message(message):
    ...
    """
    # TODO: این بخش باید بر اساس ساختار پیام‌های پیام‌رسان Bale توسعه یابد.
    # برای نمونه فعلاً None برمی‌گردد
    return None
...
def process_update(update):
    try:
        ...
        if not economic_plan.can_process():
            send_message(chat_id, "تعداد درخواست‌ها بیش از حد مجاز است، لطفاً کمی بعد تلاش کنید.")
            logger.info(f"Rate limit exceeded for chat_id={chat_id}")
            return

        image_data = get_image_data_from_message(message)
        if image_data is None:
            send_message(chat_id, "داده تصویری یافت نشد یا فرمت آن پشتیبانی نمی‌شود.")
            return
...
def worker():
    """
    پردازش پیام‌ها از صف و اجرای process_update
    """
    while True:
