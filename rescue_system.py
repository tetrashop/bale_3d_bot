...
class RescueSystem:
    ...
    def _worker_loop(self):
        ...
        while not self.stop_event.is_set():
            ...
            if not success:
                # وقتی ارسال پیام ناموفق است پیام را دوباره صف‌بندی کن
                self.logger.warning(
                    f"Send failed. Re-queue message to chat_id {chat_id}"
                )
                self.message_queue.put((chat_id, text))
                time.sleep(backoff)
                backoff = min(backoff * 2, 30)  # اکسپوننشیال بک‌آف تا ۳۰ ثانیه
            else:
                backoff = 1
    ...
    def run_fallback_check_loop(self, interval=60):
        """
        به صورت دوره‌ای تلاش می‌کند پیام‌های آفلاین ذخیره شده در bale_api را ارسال کند.
        """
        while not self.stop_event.is_set():
            try:
