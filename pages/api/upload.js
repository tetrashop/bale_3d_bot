...
export default async function handler(req, res) {
  ...
  try {
    const { image, filename, zScale = 0.5 } = req.body;
    ...
    const outputObjPath = path.join(tempDir, `${baseName}_${Date.now()}.obj`); // ذخیره در tmp
    ...
    // پاکسازی فایل‌های موقت (تصویر و OBJ)
    ...
    res.status(200).json({ success: true, token, objContent });
  } catch (error) {
    console.error(error);
